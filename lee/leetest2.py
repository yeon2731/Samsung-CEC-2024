...
class DecoderLayerBlock(nn.Module):
    ...
    def forward(self, x):
        print(f"decoderlayerblock x.shape : {x.shape}")
        residual = x
...
class NormalizationBlock(nn.Module):
    ...
    def forward(self, x):
        print(f"normalizationblock x.shape : {x.shape}")
        return self.norm(x)
...
class OutputBlock(nn.Module):
    ...
    def forward(self, x):
        print(f"outputblock x.shape : {x.shape}")
...
class Phi3ForCausalLM_Blockwise(nn.Module):
    ...
    def pipeline_process(self, inputs, batch_size=1):
        ...
        for batch in data_loader:
            ...
            with ThreadPoolExecutor(max_workers=len(self.blocks)) as executor:
                futures = [executor.submit(self._process_single_input, x) for x in batch]
    ...
    def _process_single_input(self, input_ids):
        ...
        for i, block in enumerate(self.blocks):
            x = block(x)
            print(f"Block {i+1} processed for input {input_ids}")
        return x
