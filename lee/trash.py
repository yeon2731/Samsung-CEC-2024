...
print(data[0])  # 첫 번째 요소를 확인

# 메시지를 토큰화하여 input_ids 리스트로 변환
input_ids_list = [
    tokenizer(item["message"][0]["content"], return_tensors="pt", padding="max_length",truncation=True,max_length=512).input_ids.to(model.device)
    for item in data
]
